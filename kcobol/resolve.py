...
def _resolve_name(node, path, attrs):
    ...
    if name_node.text in node.program_node.namespace:
        attrs['nodes'].add(node.program_node.namespace[name_node.text])
    elif name in node.rununit_node.namespace:
        attrs['nodes'].add(node.rununit_node.namespace[name_node.text])
    else:
